...
export async function done() {
    let html = await fetch("https://book.qidian.com/info/1027339371", {
        method: "GET",
        headers: new Headers({
            "Accept": "*/*",
        ...
        for (let key of temp) { // update detail
            while (stack.length) {
                ...
                if (top.attr && top.attr.length) {
                    let check = top.attr.filter((item) => {
    ...
    let title = res.attr.filter((item) => item.name === "title");
    // console.log(title)
